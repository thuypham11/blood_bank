...
export const getAllLabs = async (req, res) => {
	try {
		...
		const stocks = await Blood.find({
			$or: [{ bloodLab: { $in: labIds } }, { hospital: { $in: labIds } }],
		})
			.select("bloodLab hospital bloodGroup bloodType quantity status expiryDate expirationDate")
			.lean();
			...
			.map((lab) => {
				...
				return {
					...
					proximity: {
						...
						source:
							hospitalCoords?.source === "exact" && labCoords?.source === "exact" ? "exact" : "estimated",
						label:
							distanceKm !== null
			...
			.sort((a, b) => {
				if (a.proximity.distanceKm !== null && b.proximity.distanceKm !== null) {
					return (
						...
						b.stockSummary.totalUnits - a.stockSummary.totalUnits
					);
				}
				return (
					b.proximity.score - a.proximity.score || b.stockSummary.totalUnits - a.stockSummary.totalUnits
				);
			});
