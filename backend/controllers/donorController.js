...
// 📌 Lấy kết quả xét nghiệm của donor
...
// ⏰ Nhắc hẹn tái hiến (dựa trên lastDonationDate)
...
// 📜 Lấy chứng nhận cho một lần hiến cụ thể
...
// 🚨 Yêu cầu máu khẩn cấp (theo nhóm máu của donor)
export const getUrgentBloodRequests = async (req, res) => {
  try {
    const donorId = req.donor.id;
    const donor = await Donor.findById(donorId);
    if (!donor) return res.status(404).json({ message: "Donor not found" });
    // Giả sử có model BloodRequest
    ...
    }).populate("hospitalId", "name").limit(5);
