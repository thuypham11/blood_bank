...
const LabManageRequests = () => {
	...
	const getHospitalInfo = (request) => {
		return request.hospitalId || request.hospital || {};
	};
					...
					{requests.length === 0 ? (
						...
						<div className="overflow-x-auto">
							<table className="w-full">
								...
								<tbody>
									{requests.map((req) => {
										const hospital = getHospitalInfo(req);
													...
													<div className="flex items-center gap-3">
														<div className="w-10 h-10 bg-red-100 rounded-full flex items-center justify-center">
															<span className="font-semibold text-red-600">
																{hospital.name?.charAt(0) || "B"}
															</span>
														</div>
														<div>
															<div className="font-medium text-gray-800">
																{hospital.name || "Bệnh viện không xác định"}
															</div>
															<div className="flex items-center gap-1 text-sm text-gray-500">
																<MapPin size={12} />
																{hospital.address?.city || hospital.address || "Thành phố không xác định"}
															</div>
														</div>
													</div>
												</td>
