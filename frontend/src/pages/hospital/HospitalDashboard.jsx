...
const HospitalDashboard = () => {
	...
	useEffect(() => {
		const fetchHospitalData = async () => {
			try {
				...
				console.log("Đã truy cập route getProfile của cơ sở!");
				...
				console.log("Phản hồi API bệnh viện:", profileData);

				const h = profileData.hospital || profileData.facility || profileData;

				if (!h) {
					throw new Error("Không tìm thấy dữ liệu bệnh viện trong phản hồi");
				}
	...
	const getLoginHistory = () => {
		...
		return hospital.history
			...
			.map((login) => ({
				date: login.date,
				description: login.description || "Đăng nhập hệ thống",
				ip: login.description?.match(/\d+\.\d+\.\d+\.\d+/)?.[0] || "Đăng nhập cơ sở",
			}));
	};
			...
			<div className="min-h-screen bg-gradient-to-br from-red-50 to-white p-6">
				<div className="max-w-7xl mx-auto">
					<div className="flex justify-center items-center py-12">
						...
						<span className="ml-3 text-gray-600">Đang tải bảng điều khiển bệnh viện...</span>
					</div>
				</div>
			</div>
		);
		...
		return (
			<div className="min-h-screen bg-gradient-to-br from-red-50 to-white p-6">
				<div className="max-w-7xl mx-auto text-center py-12">
					...
					<h2 className="text-2xl font-bold text-gray-800 mb-2">Không thể tải dữ liệu bệnh viện</h2>
					<p className="text-gray-600 mb-4">Vui lòng thử làm mới trang hoặc liên hệ hỗ trợ.</p>
					<button
	...
	return (
		<div className="min-h-screen bg-gradient-to-br from-red-50 to-white p-6">
			<div className="max-w-7xl mx-auto">
				{/* Tiêu đề */}
				<div className="mb-8">
					<h1 className="text-3xl font-bold text-gray-800 mb-2">Bảng Điều Khiển Bệnh Viện</h1>
					<p className="text-gray-600">Chào mừng trở lại! Đây là tổng quan về bệnh viện của bạn.</p>
				</div>

				{/* Thẻ thông tin bệnh viện */}
								...
								<div>
									...
									<div className="flex flex-wrap gap-4 mt-3">
										...
										<p className="text-gray-600">
											Phân loại: <span className="font-medium">{hospital.category}</span>
										</p>
									</div>
								</div>
								...
								<div className="text-center md:text-right">
									<div className="inline-flex items-center gap-2 bg-green-100 text-green-800 px-3 py-1 rounded-full text-sm font-medium">
										...
										{hospital.status === "approved"
											? "ĐÃ PHÊ DUYỆT"
											: hospital.status?.toUpperCase() || "HOẠT ĐỘNG"}
									</div>
									<p className="text-sm text-gray-500 mt-2">
										Đăng nhập lần cuối:{" "}
										{hospital.lastLogin ? new Date(hospital.lastLogin).toLocaleString("vi-VN") : "Chưa có"}
									</p>
								</div>
							</div>
						</div>
					...
					<div className="bg-white rounded-2xl shadow-lg border border-blue-100 p-6">
						...
						{requests.length === 0 ? (
							<div className="text-center py-8">
								...
								<p className="text-gray-600">Chưa có yêu cầu máu nào</p>
							</div>
						) : (
							<div className="space-y-3">
								...
								{loginHistory.map((login, index) => (
									<div
										...
										<div>
											...
											<div className="text-sm text-gray-600">
												{new Date(login.date).toLocaleString("vi-VN")}
											</div>
										</div>
