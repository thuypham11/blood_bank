...
const EMPTY_FORM = {
  ...
  venue: "", address: "", city: "TP. Hồ Chí Minh", state: "",
  hospital: "", expectedDonors: 100, description: "",
  lat: "", lng: "",
};

// Geocode địa chỉ → tọa độ qua Nominatim (miễn phí)
const geocodeAddress = async (venue, address, city) => {
  const query = [venue, address, city, "Việt Nam"].filter(Boolean).join(", ");
  try {
...
const AdminBloodCamps = () => {
  ...
  // ── Geocode khi địa chỉ thay đổi (debounce 800ms) ──────────────
  const geocodeTimer = useRef(null);
  const triggerGeocode = useCallback(() => {
    if (!form.venue && !form.city) return;
    clearTimeout(geocodeTimer.current);
    geocodeTimer.current = setTimeout(async () => {
      setGeocoding(true);
      const coords = await geocodeAddress(form.venue, form.address, form.city);
      setGeocoding(false);
      if (coords) {
        setMapCoords(coords);
  ...
  }, [form.venue, form.address, form.city]);

  useEffect(() => { if (showCreateModal) triggerGeocode(); }, [form.venue, form.address, form.city, showCreateModal]);
  ...
  const openGoogleMaps = () => {
    const query = [form.venue, form.address, form.city, "Việt Nam"].filter(Boolean).join(", ");
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl my-8 p-6">
            ...
            <form onSubmit={handleSubmitForm} className="space-y-4">
              ...
              <div>
                ...
                <input placeholder="VD: 36 Lê Quý Đôn, Phường 6" value={form.address}
