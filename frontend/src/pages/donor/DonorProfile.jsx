...
// ── Đặt NGOÀI component để tránh tạo lại mỗi lần render (nguyên nhân màn đen) ──
const VALIDATION_RULES = {
	fullName: { required: true, minLength: 2, maxLength: 200 },
	phone: { required: true, pattern: /^[0-9]{10}$/ }, // Model: 10 chữ số
	age: { required: true, min: 18, max: 65 },
	gender: { required: true },
	weight: { required: false, min: 45, max: 200 }, // Model: không required
	bloodGroup: { required: true },
	"address.street": { required: true, minLength: 1 }, // Model: required
	"address.city": { required: true, minLength: 1 }, // Model: required
	"address.state": { required: true, minLength: 1 }, // Model: required
	"address.pincode": { required: true, pattern: /^[0-9]{6}$/ }, // Model: 6 chữ số
	password: { minLength: 8 }, // Model: tối thiểu 8 ký tự
};
...
// Helper: luôn trả về string từ bất kỳ dạng lỗi nào
...
const DonorProfile = () => {
	...
	const fetchProfile = useCallback(async () => {
		try {
			...
			// Kiểm tra data.donor TRƯỚC khi truy cập bất kỳ field nào
			const donorData = data?.donor || data;
			if (!donorData) throw new Error("Không nhận được dữ liệu hồ sơ từ server.");

			// Chuẩn hóa gender khớp với enum model: "Male" | "Female"
			...
			console.error("❌ Lỗi tải hồ sơ người hiến máu:", error);

			if (error.message?.includes("Không tìm thấy token xác thực") || error.response?.status === 401) {
	...
	const handleSave = async () => {
		...
		try {
			...
			// Safe trim: tránh crash khi giá trị là undefined/null
			...
			console.log("📥 Response từ backend:", JSON.stringify(data, null, 2));
			...
			if (data.success || data.donor || data.message === "Profile updated successfully") {
				...
				setFormData({
					...
					gender:
						(updatedDonor.gender || payload.gender || "").charAt(0).toUpperCase() +
						(updatedDonor.gender || payload.gender || "").slice(1).toLowerCase(),
			...
			// Parse lỗi Mongoose validation từ backend (400 Bad Request)
			// Backend trả về errors dạng: { fieldName: { name, message, properties, kind, path, value } }
			// hoặc dạng array, hoặc dạng string đơn giản
			...
			if (backendErrors) {
				...
				if (typeof backendErrors === "object" && !Array.isArray(backendErrors)) {
					...
					Object.entries(backendErrors).forEach(([field, errObj]) => {
						// Lấy .message nếu là object, hoặc dùng trực tiếp nếu đã là string
						parsedErrors[field] =
							typeof errObj === "string" ? errObj : errObj?.message || "Giá trị không hợp lệ";
					});
			...
			const message =
				...
				error.message ||
				"Không thể lưu thay đổi. Vui lòng kiểm tra kết nối mạng.";
			toast.error(typeof message === "string" ? message : "Lỗi không xác định.");
		} finally {
			setSaving(false);
		}
	};
	...
	const handleCancel = () => {
		...
		if (donor) {
			setFormData({
				...
				age: donor.age || "",
				gender:
					(donor.gender || "").charAt(0).toUpperCase() + (donor.gender || "").slice(1).toLowerCase(),
				weight: donor.weight || "",
						...
						<div className="flex items-center gap-4">
							...
							<div>
								<h1 className="text-3xl font-bold text-gray-800">
									{donor.fullName || "Hồ Sơ Người Hiến Máu"}
								</h1>
								<p className="text-gray-600 mt-1 flex items-center gap-2">
									<Droplet size={16} className="text-red-500" />
									{donor.bloodGroup || "Người Hiến Máu"} •{" "}
									<span className="font-mono text-sm">Mã: {donor.donorId}</span>
								</p>
							</div>
						</div>
							...
							<div className="space-y-4">
								...
								<div className="flex justify-between items-center">
									...
									<span className="text-sm font-bold text-red-600">
										{donor.bloodGroup || "Chưa cập nhật"}
									</span>
								</div>
							...
							<div className="space-y-3">
								...
								{donor.age && (
									<div className="flex items-center gap-3 text-sm">
										...
										<span className="text-gray-600">{donor.age} tuổi</span>
									</div>
								)}
							</div>
						</div>
											...
											<input
												...
												className={`w-full px-4 py-3 rounded-xl border ${
													isEditing
														? "border-gray-300 bg-white focus:ring-2 focus:ring-red-500 focus:border-red-500"
														: "bg-gray-50 border-gray-200"
												} ${errors[`address.${key}`] ? "border-red-500" : ""}`}
												placeholder={`Nhập ${label.toLowerCase()}`}
											/>
											{errors[`address.${key}`] && (
												<p className="text-red-500 text-xs mt-2 flex items-center gap-1">
													<AlertCircle size={12} />
													{getErrorMsg(errors[`address.${key}`])}
												</p>
											)}
										</div>
									))}
								...
								<div className="mb-8">
									...
									<div className="max-w-md">
										...
										<input
											...
											onChange={handleChange}
											className={`w-full px-4 py-3 rounded-xl border border-gray-300 bg-white focus:ring-2 focus:ring-red-500 focus:border-red-500 ${
